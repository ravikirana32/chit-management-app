...
export class AuctionStateService {
  ...
  async getState(auctionId: string, userId: string) {
    ...
    const [access]:any=await this.sequelize.query(`
      ...
      LIMIT 1`,{replacements:{chitId:a.chit_id,userId}});
    if(!access.length)throw new NotFoundException('Auction not found');
    ...
    const localToday=new Intl.DateTimeFormat('en-CA',{timeZone:a.timezone||'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
