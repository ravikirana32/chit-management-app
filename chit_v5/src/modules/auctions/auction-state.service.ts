...
export class AuctionStateService {
  ...
  async getState(auctionId: string) {
    ...
    const localToday=new Intl.DateTimeFormat('en-CA',{timeZone:process.env.APP_TIMEZONE||'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
    const dateReady=bypass||!scheduledDate||scheduledDate===localToday;
    const status=String(a.status||'').toUpperCase();
    const allowedActions={
      open:false,bid:status==='OPEN',close:status==='OPEN'&&(bypass||now<closeMs),reopen:status==='CLOSED_PENDING_FINALIZATION'&&(bypass||dateReady),finalize:status==='CLOSED_PENDING_FINALIZATION'||(status==='OPEN'&&now>=closeMs),additionalOpen:false
    };
