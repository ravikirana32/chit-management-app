...
class DrawsController {
  ...
  start(@Param('chitId') chitId:string,@Body() dto:StartDrawDto,@CurrentUser() user:any){
  ...
  interest(@Param('chitId') chitId:string,@Param('monthId') monthId:string,@Body() dto:DrawInterestDto,@CurrentUser() user:any){
  ...
  get(@Param('chitId') chitId:string,@Param('monthId') monthId:string,@CurrentUser() user:any){
    return this.service.getDraw(chitId,monthId,user.sub);
  }

  @Post('chits/:chitId/months/:monthId/run')
  @ApiOperation({summary:'Agent executes the FIXED_DRAW after the interest window'})
  run(@Param('chitId') chitId:string,@Param('monthId') monthId:string,@CurrentUser() user:any){
  ...
  agentPayout(@Param('chitId') chitId:string,@Param('monthId') monthId:string,@CurrentUser() user:any){
    return this.service.runAgentChit(chitId,monthId,user.sub);
  }
}

@Module({controllers:[DrawsController],providers:[FixedDrawService],exports:[FixedDrawService]})
