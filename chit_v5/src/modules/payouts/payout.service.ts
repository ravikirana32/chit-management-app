...
export class PayoutService {
  ...
  async list(chitId:string,userId:string){
    ...
    if(!access.length) throw new ConflictException('Only creator can view payout register');
    ...
    const [rows]:any=await this.sequelize.query(
      `SELECT p.*,u.name AS recipient_name,u.mobile_number AS recipient_mobile
       ...
       WHERE p.chit_id=:chitId ORDER BY p.created_at DESC`,
      {replacements:{chitId}});
    return rows;
  }
  ...
  async settle(payoutId:string,actor:string,dto:any){
    return this.sequelize.transaction(async transaction=>{
      const [rows]:any=await this.sequelize.query(
        `SELECT p.*,c.creator_id FROM payouts p
         JOIN chits c ON c.id=p.chit_id
         WHERE p.id=:payoutId FOR UPDATE`,
        {replacements:{payoutId},transaction});
      if(!rows.length) throw new NotFoundException('Payout not found');
      const p=rows[0];

      if(p.creator_id!==actor) throw new ConflictException('Only creator can settle payout');
      if(!['SETTLED','FAILED'].includes(dto.status)) throw new BadRequestException('Invalid payout status');
      if(p.status==='SETTLED') throw new ConflictException('Payout already settled');
      ...
      const [updated]:any=await this.sequelize.query(
        `UPDATE payouts
         ...
         RETURNING *`,
        {replacements:{
          payoutId,status:dto.status,method:dto.paymentMethod,
          reference:dto.transactionReference,notes:dto.notes??null
        },transaction});
      ...
      if(dto.status==='SETTLED'){
        const [already]:any=await this.sequelize.query(
          ...
          {replacements:{payoutId},transaction});
        if(already.length) throw new ConflictException('Payout ledger entry already exists');
        await this.sequelize.query(
          `INSERT INTO ledger_entries
           ...
           SELECT gen_random_uuid(),p.chit_id,p.chit_month_id,cp.id,'PAYOUT',
                  -p.amount,'Payout settled',
                  'PAYOUT',p.id,:actor,NOW(),NOW()
           FROM payouts p
