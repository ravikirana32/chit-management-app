    ...
    return this.sequelize.transaction(async transaction=>{
      const [rows]:any=await this.sequelize.query(
        `SELECT m.*,c.creator_id FROM chit_months m JOIN chits c ON c.id=m.chit_id
         WHERE m.id=:monthId FOR UPDATE`,
        {replacements:{monthId},transaction});
      if(!rows.length) throw new NotFoundException('Month not found');
      if(rows[0].creator_id!==userId) throw new ConflictException('Only creator can close month');
      if(rows[0].status==='LOCKED') return rows[0];
      const [open]:any=await this.sequelize.query(
        ...
        {replacements:{monthId},transaction});
      if(open[0].count>0) throw new ConflictException('Cannot lock month with unresolved contribution obligations');
      ...
      const [payout]:any=await this.sequelize.query(
        ...
        {replacements:{monthId},transaction});
      if(payout[0].count>0) throw new ConflictException('Cannot lock month with pending payout');

      const [updated]:any=await this.sequelize.query(
        `UPDATE chit_months SET status='LOCKED',locked_at=NOW(),locked_by=:userId,updated_at=NOW()
         WHERE id=:monthId RETURNING *`,
