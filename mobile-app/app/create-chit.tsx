...
export default function CreateChit(){
 ...
 const submit=async()=>{
  ...
  if(isAdmin(user)&&!agentId)return Alert.alert('Agent required','Select the active agent who will be responsible for this chit.');
  ...
  try{
   const payload:any={name:name.trim(),description:description.trim()||undefined,chitType:type,totalMembers:nMembers,totalMonths:nMonths,startDate:start,dueDay:Math.min(28,Math.max(1,Number(due)||1)),creatorParticipates,
    ...
    agentMonthNumbers:agentMonths,agentId:agentId||undefined};
 ...
 <Card><Text style={s.section}>New chit or existing running chit?</Text><Text style={s.muted}>Use the existing-chit flow when this chit already has completed months outside the app. Historical months will be locked and the next active month will continue normally.</Text><Button title="Onboard Existing / Running Chit" secondary onPress={()=>router.push('/existing-chit')}/></Card>
