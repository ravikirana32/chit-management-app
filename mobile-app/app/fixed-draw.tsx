...
export default function FixedDraw(){
 ...
 const run=async()=>{setBusy(true);try{await drawsApi.run(String(chitId),String(monthId));await load()}catch(e){const message=errMsg(e);if(message.toLowerCase().includes('fully verified')||message.toLowerCase().includes('unresolved contribution')||message.toLowerCase().includes('awaiting verification'))Alert.alert('Draw blocked','All member contributions must be fully verified before running the draw.',[{text:'Go to Collections',onPress:()=>router.push({pathname:'/collections',params:{chitId:String(chitId)}})},{text:'OK',style:'cancel'}]);else Alert.alert('Run failed',message)}finally{setBusy(false)}};
