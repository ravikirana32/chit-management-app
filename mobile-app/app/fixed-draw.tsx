...
export default function FixedDraw(){
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   try{const d=await drawsApi.get(String(chitId),String(monthId));setState(d.data?.data??d.data)}
 ...
 const revealActive=revealStatus==='REVEALING'; const completed=revealStatus==='REVEALED';
 ...
 return <Screen title="Fixed Draw" subtitle="Interest → secure selection → winner reveal" back={()=>router.back()}><ScrollView>
  ...
  {!revealActive&&!completed&&operate&&state.status!=='COMPLETED'&&<Button title="Run Draw Now" onPress={run} disabled={busy}/>}
 </ScrollView></Screen>;
}
