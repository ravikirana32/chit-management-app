 ...
 return <Screen title={String(chit.name)} subtitle={`${chit.chit_type} · ${status}`} back={()=>router.back()}><ScrollView>
   ...
   <Card><Text style={s.section}>Monthly schedule</Text>{months.map((m:any)=>{
     ...
     return <Card key={m.id}><View style={s.row}><View style={{flex:1}}><Text style={{fontWeight:'800'}}>Month {m.month_number}{status==='ACTIVE'&&current?' · CURRENT':''}</Text><Text style={s.muted}>{date(m.scheduled_date)} · {money(m.scheduled_amount)} / member</Text><Text style={s.muted}>{historical?'Historical month — locked':m.month_type==='AGENT_CHIT'?'Agent month — no draw/bid':`Action month — ${chit.chit_type==='AUCTION'?'auction':'fixed draw'}`}</Text></View><Badge tone={monthStatus==='COMPLETED'||monthStatus==='LOCKED'?'green':'orange'}>{monthStatus||'SCHEDULED'}</Badge></View><Text>Payout: {money(m.winner_payout_amount)}</Text>
       ...
       {status==='ACTIVE'&&current&&!closed&&isActionMonth&&monthType!=='AGENT_CHIT'&&chit.chit_type==='FIXED_DRAW'&&<Button title="Open / Manage Fixed Draw" secondary onPress={()=>router.push({pathname:'/fixed-draw',params:{chitId:String(chit.id),monthId:String(m.id)}})}/>}
       {status==='ACTIVE'&&current&&!closed&&isActionMonth&&monthType!=='AGENT_CHIT'&&chit.chit_type==='AUCTION'&&<Button title="Open / Manage Auction" secondary onPress={()=>router.push({pathname:'/auction',params:{chitId:String(chit.id),monthId:String(m.id)}})}/>}
       {status==='ACTIVE'&&current&&!closed&&monthType==='AGENT_CHIT'&&<Button title="Agent Month / Settlement" secondary onPress={()=>router.push({pathname:'/agent-month',params:{chitId:String(chit.id),monthId:String(m.id)}})}/>}
       ...
       {(manage||isAdmin(user)||creator)&&(monthStatus==='COMPLETED'||historical)&&<Button title="Reconciliation" secondary onPress={()=>router.push({pathname:'/reconciliation',params:{chitId:String(chit.id),monthId:String(m.id)}})}/>}
