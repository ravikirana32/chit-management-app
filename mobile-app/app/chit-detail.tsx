...
export default function ChitDetail(){
 ...
 const currentMonth=useMemo(()=>months.find((m:any)=>!CLOSED_MONTH_STATUSES.has(String(m.status||'').toUpperCase())),[months]);
 ...
 return <Screen title={String(chit.name)} subtitle={`${chit.chit_type} · ${status}`} back={()=>router.back()}><ScrollView>
   ...
   {status==='ACTIVE'&&<Card><Text style={s.section}>Chit Active</Text>
     ...
     {hasOpenMonth&&<Button title={`Go to Month ${currentMonth.month_number} Operations`} onPress={()=>{}} secondary/>}
   </Card>}
