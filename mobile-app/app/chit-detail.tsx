...
export default function ChitDetail(){
 ...
 const[chit,setChit]=useState<any>();const[access,setAccess]=useState<any>();const[dash,setDash]=useState<any>();const[expandedMonth,setExpandedMonth]=useState<string|null>(null);const[busy,setBusy]=useState(false);
 ...
 const start=async()=>{setBusy(true);try{await chitsApi.start(String(chit.id));Alert.alert('Chit started','The chit is now ACTIVE and Month 1 is ready for operations.');await load()}catch(e){Alert.alert('Start failed',errMsg(e))}finally{setBusy(false)}};
 return <Screen title={String(chit.name)} subtitle={`${chit.chit_type} · ${status}`} back={()=>router.back()}><ScrollView>
   ...
   {status==='READY_TO_START'&&<Card><Text style={s.section}>Published — Ready to Start</Text><Text style={s.muted}>Configuration is locked. Start the chit when you are ready to begin Month 1.</Text><Button title="Start Chit / Month 1" onPress={start} disabled={busy}/></Card>}
   ...
   <Card><Text style={s.section}>Monthly schedule</Text>{(chit.months||[]).map((m:any)=>{const action=m.month_type==='ACTION';const active=!['LOCKED','COMPLETED'].includes(String(m.status||'').toUpperCase());const current=Number(m.month_number)===Number(chit.completed_months||0)+1;
     return <Card key={m.id}><View style={s.row}><View style={{flex:1}}><Text style={{fontWeight:'800'}}>Month {m.month_number}{status==='ACTIVE'&&current?' · CURRENT':''}</Text><Text style={s.muted}>{date(m.scheduled_date)} · {money(m.scheduled_amount)} / member</Text><Text style={s.muted}>{m.month_type==='AGENT_CHIT'?'Agent month — no draw/bid':`Action month — ${chit.chit_type==='AUCTION'?'auction':'fixed draw'}`}</Text></View><Badge tone={m.status==='COMPLETED'?'green':'orange'}>{String(m.status)}</Badge></View><Text>Payout: {money(m.winner_payout_amount)}</Text>
       ...
       {['COMPLETED','LOCKED'].includes(String(m.status||'').toUpperCase())&&<Button title={expandedMonth===m.id?'Hide complete month details':'View complete month details'} secondary onPress={()=>setExpandedMonth(expandedMonth===m.id?null:m.id)}/>}
       {expandedMonth===m.id&&<Card><Text style={s.section}>Month details</Text><Text>Month status: {String(m.status||'—')}</Text><Text>Month type: {m.month_type==='AGENT_CHIT'?'Agent month':'Fixed draw'}</Text><Text>Scheduled date: {date(m.scheduled_date)}</Text><Text>Contribution / member: {money(m.scheduled_amount)}</Text><Text>Expected collection: {money(Number(m.scheduled_amount||0)*Number(chit.total_members||0))}</Text><Text>Verified collections: {money(m.verified_collections||0)}</Text><Text>Configured payout: {money(m.winner_payout_amount)}</Text><Text>Payout status: {m.payout_status||'No payout record'}</Text><Text>Paid toward payout: {money(m.payout_paid_amount||0)}</Text><Text>Remaining payout: {money(m.payout_remaining_amount??m.winner_payout_amount)}</Text>{m.payout_recipient_name&&<Text>Winner / recipient: {m.payout_recipient_name}</Text>}<Text>Settlement payments: {Number(m.payout_settlement_count||0)}</Text>{(m.payout_settlements||[]).map((x:any)=><View key={x.id} style={{paddingVertical:6,borderBottomWidth:1,borderBottomColor:'#ddd'}}><Text style={{fontWeight:'800'}}>{money(x.amount)} · {x.payment_method}</Text><Text style={s.muted}>{x.transaction_reference} · {x.paid_at?new Date(x.paid_at).toLocaleString():'—'}</Text></View>)}{m.locked_at&&<Text>Locked at: {new Date(m.locked_at).toLocaleString()}</Text>}</Card>}
