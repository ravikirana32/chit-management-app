...
export default function ChitDetail(){ 
 ...
 return <Screen title={String(chit.name)} subtitle={`${chit.chit_type} · ${status}`} back={()=>router.back()}><ScrollView>
   ...
   <Card><Text style={s.section}>Monthly schedule</Text>{months.map((m:any)=>{
     const monthStatus=String(m.status||'').toUpperCase();const closed=CLOSED_MONTH_STATUSES.has(monthStatus);const historical=String(m.data_origin||'').toUpperCase()==='HISTORICAL';const current=!!currentMonth&&String(m.id)===String(currentMonth.id);const action=m.month_type==='ACTION';
