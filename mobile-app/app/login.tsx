...
export default function Login(){
  ...
  const request=async()=>{
    ...
    try{
      ...
      Alert.alert('OTP requested',d?.devOtp?`Development OTP: ${d.devOtp}`:'Enter the OTP sent to your mobile.');
  ...
  const verify=async(code:string)=>{
    ...
    setBusy(true);
    try{
      await login(mobile.trim(),code.trim());
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      ...
      >
        <View style={{marginTop:60}}>
          ...
          {dev&&<View style={{padding:14,borderWidth:1,borderRadius:12,marginBottom:18}}>
            <Text style={{fontWeight:'800'}}>DEVELOPMENT MODE</Text>
            <Text style={s.muted}>OTP is still issued and verified through the backend challenge flow.</Text>
          </View>}
          <Input label="Mobile number" value={mobile} onChangeText={setMobile} placeholder="+919999999999" keyboardType="phone-pad" returnKeyType="done" onSubmitEditing={Keyboard.dismiss}/>
          <Button title={sent?'Resend OTP':'Request OTP'} onPress={request} disabled={busy}/>
          {sent&&<View>
            <Input label="OTP" value={otp} onChangeText={setOtp} placeholder={devOtp||'123456'} keyboardType="number-pad" returnKeyType="done" onSubmitEditing={Keyboard.dismiss}/>
            <Button title="Verify & Login" onPress={()=>verify(otp||devOtp)} disabled={busy}/>
          </View>}
        </View>
      </ScrollView>
